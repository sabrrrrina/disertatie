...
export async function getAIblock(snippets) {

    const content_message = `
    You are a content filter for a browser extension.
    Decide whether a snippet belongs to one or more of these categories:
    - violence
    - hate
    - gambling
    - drugs
    ...
    [
    {"xpath": "...", "categories": ["violence"]},
    {"xpath": "...", "categories": []}
    ]`;

    const user_message = "Here are the snippets:\n";
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
            ...
            messages: [
                ...
                { role: "user", content: user_message + JSON.stringify(snippets, null, 2) }
            ]
        })
    });

    const data = await response.json();
    console.log("AI block raw:", JSON.stringify(data, null, 4));
    return data.choices[0].message.content;
}
